...
usage() {
  echo "Usage: $0 --solver-json <solver_json> [--auth-token <auth_token>] [--url <url>]"
  echo ""
  echo "Options:"
...
solver=$(curl -f -X POST "http://${url}/api/v1/solver" -H 'Content-Type: application/json' -H "${auth_header}" -d "@${solver_json}")

echo "${solver}"
echo "${solver}" >> created_solvers.txt
