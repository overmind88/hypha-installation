...
# from version 2024.1 to version 2025.1
...
show_help() {
  ...
  echo "  --archive=path/to/archive  Path to the local archive contains 2025.1 installation scripts."
  ...
  echo "  $0 --archive=2025.1.zip                     # Uses provided zip file for migration"
  echo "  $0 --env=config/.env                             # Specifies path to .env file"
  echo "  $0 --archive=2025.1.zip --env=config/.env   # Specifies path to .env file"
}
...
echo "HYPHA_3D_3D_SERVICE_DB_USERNAME=models" >> ${env_file}
echo "HYPHA_3D_3D_SERVICE_DB_PASSWORD=models" >> ${env_file}
...
HYPHA_3D_3D_SERVICE_DB_USERNAME
HYPHA_3D_3D_SERVICE_DB_PASSWORD
