...
# from version 2023.2 to version 2023.3
# You must use docker-compose.yml file for 2023.2 version

set -e

# Default URL of the installation script on GitHub
GITHUB_URL="https://github.com/mycesys/hypha-installation/archive/refs/heads/2023.3.zip"

NEW_VERSION_SOURCES=2023.3_"$(date '+%s')"
...
show_help() {
  ...
  echo
  echo "This script migrates from version 2023.2 to 2023.3."
  ...
  echo "  --archive=path/to/archive  Path to the local archive contains 2023.3 installation scripts."
  ...
  echo "  $0 --archive=2023.3.zip                     # Uses provided zip file for migration"
  echo "  $0 --env=config/.env                             # Specifies path to .env file"
  echo "  $0 --archive=2023.3.zip --env=config/.env   # Specifies path to .env file"
}
...
cp "$UNZIP_DIR"/hypha-installation-2023.3/allinone/prepare-dirs.sh ./
cp "$UNZIP_DIR"/hypha-installation-2023.3/allinone/docker-compose.yml ./
cp -r "$UNZIP_DIR"/hypha-installation-2023.3/allinone/vault_config ./

#### Creating a backup for existing .env file

backupenvfile="$env_file".2023.3-"$(date '+%s')".backup
